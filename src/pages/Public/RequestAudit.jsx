...
const SERVICES = [
  ...
  { id: 'COMPLIANCE', title: 'Regulatory & Business KYC', desc: 'BRS and tax compliance verification.' },
]
...
export default function RequestAudit() {
  ...
  async function handleSubmit(e) {
    ...
    setTimeout(() => {
      setLoading(false)
      toast.success('Audit request received! Our engineering team will contact you within 24 hours.')
      ...
      <div className="relative mx-auto max-w-3xl">
        ...
        >
          ...
          <h1 className="font-display text-3xl font-semibold tracking-tight md:text-4xl">
            Request a <span className="gradient-text">Security Audit</span>
          </h1>
          <p className="mt-3 text-ink-light/60 dark:text-ink-dark/60 max-w-lg mx-auto text-sm md:text-base">
            Have our expert engineers review your system architecture, software, or smart contracts before issuing your official verification seal.
          </p>
        </motion.div>
        ...
        >
                ...
                return (
                  <button
                    ...
                    className={`relative text-left rounded-2xl p-4 border transition-all duration-200 ${
                      isSelected
                        ? 'border-brand-violet bg-brand-violet/10 shadow-glow'
                        : 'border-black/10 dark:border-white/10 hover:bg-black/[0.02] dark:hover:bg-white/[0.04]'
                    }`}
                  >
                    {isSelected && (
                      <span className="absolute top-3 right-3 h-5 w-5 rounded-full bg-brand-violet text-white flex items-center justify-center">
          ...
          <div>
            <label className="mb-2 flex items-center gap-2 text-sm font-medium">
              <FileCode className="h-4 w-4 text-brand-violet" />
              Additional Details / Infrastructure Notes
            </label>
            <textarea
              rows={4}
              value={form.notes}
              onChange={(e) => setForm({ ...form, notes: e.target.value })}
              placeholder="Tell us about your tech stack, system goals, or timeline..."
